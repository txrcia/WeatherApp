...
def satisfaction_prediction_page():
    st.markdown("""
    <style>
        .big-heading { font-size: 30px; font-weight: bold; text-align: center; color: white; }
        .description-box {
        ...
        .st-expanderContent { background-color: #222222 !important; }
    </style>
    ...
    if mode == "Use Manual Inputs":
        with st.form("manual_input_form"):
            ...
            for feature in X.columns:
                if feature in categorical_cols:
                    ...
                    val = st.select_slider(f"**{feature}** (1 = Poor, 5 = Excellent)", options=[1,2,3,4,5], value=3)
                    user_data[feature] = val
                else:
                    val = st.slider(f"**{feature}**", 0, 3000)
                    user_data[feature] = val
        ...
        if submitted:
            ...
            feat_df = pd.DataFrame({ 'Feature': X.columns, 'Importance': feature_importances }).sort_values(by='Importance', ascending=False)
            ...
            fig2.update_layout(plot_bgcolor='black', paper_bgcolor='black', font=dict(color='white'), height=400)
        ...
        if uploaded_file:
            ...
            if missing_cols:
                ...
                df_uploaded["Predicted Satisfaction Label"] = df_uploaded["Predicted Satisfaction"].map({ 0: "Neutral or Dissatisfied", 1: "Satisfied" })
